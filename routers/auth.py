...
async def register(data: UserRegisterWithInvite):
    ...
    reg_setting = await db_fetchone("SELECT value FROM app_settings WHERE key='registration_open'")
    if reg_setting:
        reg_open = (reg_setting.get("value","")).lower() == "true"

    if not reg_open:
        if not data.invite_code:
            raise HTTPException(400, "Registration is invite-only. Enter your invite code.")
        invite_row = await db_fetchone(
            "SELECT id, use_count, max_uses FROM invites WHERE code=? AND (expires_at IS NULL OR expires_at > datetime('now'))",
            (data.invite_code,)
        )
        if not invite_row:
            raise HTTPException(400, "Invalid or expired invite code.")
        if invite_row.get("use_count",0) >= invite_row.get("max_uses",1):
            raise HTTPException(400, "Invite code already used.")
    else:
        invite_row = None

    existing = await db_fetchone("SELECT id FROM users WHERE email = ?", (email,))
    if existing:
    ...
    user = await db_fetchone(
        "SELECT id, email, username, avatar_color, created_at, is_admin, is_active, role FROM users WHERE id=?",
        (new_id,)
    )
    if not user:
...
async def delete_invite(invite_id: int, current_user=Depends(get_current_user)):
    if not current_user or not current_user.get("is_admin"):
        raise HTTPException(403, "Admin only")
        await db_execute("DELETE FROM invites WHERE id=?", (invite_id,))
