...
async def get_macro():
    ...
    # Return both wrapped (for renderMacro) and flat (backward compat)
...
async def get_personalized_events(
    ...
    """
    if not user:
        # Unauthenticated: just return top events by severity
        async with aiosqlite.connect(settings.db_path) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute(
    ...
    async with aiosqlite.connect(settings.db_path) as db:
        ...
        # ── 1. Build affinity from activity log ─────────────────────────
        async with db.execute("""
        ...
        # ── 2. Fallback to onboarding interests ─────────────────────────
        async with db.execute(
    ...
    if total_actions >= 5:
        # Real behaviour → weight by frequency
        for row in affinity_rows:
            affinity[row[0]] = row[1] / total_actions
    else:
        # Not enough data → use onboarding interests
        profile   = dict(profile_row) if profile_row else {}
        interests = []
        try:    interests = _json.loads(profile.get("interests") or "[]")
        except: pass
        # Severity threshold by experience level (mirrors JS logic)
    ...
    # Add region filter from onboarding
    profile   = dict(profile_row) if profile_row else {}
    regions   = []
    ...
    for ev in all_events:
        ...
        cat_score = affinity.get(cat, 0.05)   # small base so nothing is zero
...
async def get_asset_drivers(
    ...
    async with aiosqlite.connect(settings.db_path) as db:
        db.row_factory = aiosqlite.Row
        # Search in related_markets JSON (stored as text) for the symbol
        ...
        """, (
            f"-{hours} hours",
            f"%{symbol_clean}%",          # exact match
            f"%{symbol_clean[:3]}%",       # prefix match (e.g. "BTC" in "Bitcoin")
            f"%{_market_alias(symbol_clean)}%",  # alias (Oil → Crude)
        )) as cur:
            rows = [_parse_ev(dict(r)) for r in await cur.fetchall()]
