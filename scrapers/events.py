...
RSS_SOURCES: List[Tuple[str, str, float]] = [
    ...
    ("FT Headlines",   "https://www.ft.com/?format=rss",                                0.91),
    ("Bloomberg",      "https://feeds.bloomberg.com/markets/news.rss",                  0.91),
    ("CNBC World",     "https://www.cnbc.com/id/100727362/device/rss/rss.html",         0.86),
...
CLASSIFY_RULES: List[Tuple[List[str], str, float, float]] = [
    ...
    (["recession","gdp","inflation rate","interest rate","imf","world bank","fiscal","monetary policy",
      "trade deficit","tariff","trade war","economic growth","unemployment rate"],
     "ECONOMICS", 6.0, 7),
    (["stock market","nasdaq","dow jones","s&p 500","earnings","ipo","central bank","fed rate",
      "bond yield","cryptocurrency","bitcoin","hedge fund","private equity"],
     "FINANCE", 5.5, 7),
    ...
    async with httpx.AsyncClient(
        ...
        for source, url, credibility in RSS_SOURCES:
            try:
                ...
                before = len(raw)
                for item in items[:25]:  # max 25 per feed
